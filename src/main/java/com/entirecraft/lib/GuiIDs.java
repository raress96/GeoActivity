package com.entirecraft.lib;

public class GuiIDs
{
	public static final int CR = 1;
	public static final int RM = 2;
	public static final int CrM = 3;
	public static final int ADV = 4;
	public static final int ACR = 5;
	public static final int AE = 6;
	public static final int ChM = 7;
	public static final int REDM = 8;
	public static final int REDBM = 9;
	public static final int ASB = 10;
	public static final int AOR = 11;
	public static final int ADVArmor = 12;
	public static final int AREDBM = 13;
	public static final int TC = 14;
	public static final int GG = 15;
}
